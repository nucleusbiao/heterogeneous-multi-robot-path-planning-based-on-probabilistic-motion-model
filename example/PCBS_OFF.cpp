...
public:
    ...
    void getNeighbors(const State& s,
                      ...
                      EdgeCost& edgeCost) {
        neighbors.clear();
        auto ecWaitfind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime+1));
        auto ecMovefind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime));
        {
            if(ecWaitfind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime));
                if(ec!=edgeCost.costTable.end()){
                    CostTable ecWait(m_agentIdx,ec->step+1,ec->waitTime+1);
                    ...
                    auto sigleAction=edgeCost.costTable.find(CostTable(m_agentIdx,0,0));
                    CostTable ecWait(m_agentIdx,ec->step+1,ec->waitTime+1,sigleAction->cost);
                    edgeCost.costTable.emplace(ecWait);
                }
            }
        ...
        {
            auto sigleAction=edgeCost.costTable.find(CostTable(m_agentIdx,0,0));
            if(ecMovefind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime));
                if(ec!=edgeCost.costTable.end()){
                    CostTable ecMove(m_agentIdx,ec->step+1,ec->waitTime);
                    std::vector<double> pdfAction;
                    triangleProbFuc(pdfAction, a*sigleAction->cost,sigleAction->cost,c*sigleAction->cost,t_Div,sigleAction->cost*2);
                    convdx(ecMove.pdf,ec->pdf,pdfAction);
                    edgeCost.costTable.emplace(ecMove);

                }else{

                    CostTable ecMove(m_agentIdx,ec->step+1,ec->waitTime,sigleAction->cost);
                    edgeCost.costTable.emplace(ecMove);
