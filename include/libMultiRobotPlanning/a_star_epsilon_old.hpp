...
purposes.
  ...
  - `void onDiscover(const State& s, int fScore, int gScore)`\n
   ...
   std::hash<State>
*/
template <typename State, typename Action,typename EdgeCost,typename CostTable, typename Cost, typename Environment,
...
class AStarEpsilon {
 public:
  ...
  bool search(const State& startState,
    ...
    //if(m_env.agentID==7) std::cout<<"bestFScore: "<<bestFScore<<std::endl;
...
// check focal list/open list consistency
      ...
      if (m_env.isSolution(current.state)) {
        ...
        if(solution.states.size()>1){
            for(size_t i=0;i<=solution.states.size()-2;i++){
                solution.states[i].first.nowAction=solution.states[i+1].first.lateAction;
                /*auto ec=edgeCost.costTable.find(CostTable(m_env.agentID));
                if(solution.states[i].first.nowAction!=Action::Wait){
                    solution.states[i].first.pdf.assign(ec->pdf.begin(),ec->pdf.end());
                }*/

            }
        }

        solution.states[solution.states.size()-1].first.nowAction=Action::Wait;
      ...
      // traverse neighbors
      neighbors.clear();
      m_env.getNeighbors(current.state, neighbors,edgeCost);
        ...
        if (closedSet.find(neighbor.state) == closedSet.end()) {
          ...
          if (iter == stateToHeap.end()) {  // Discover a new node
            ...
            Cost focalHeuristic =
                ...
                m_env.focalStateHeuristic(neighbor.state, tentative_gScore,edgeCost) +
                m_env.focalTransitionHeuristic(current.state, neighbor.state,
                                               current.gScore,
                                               tentative_gScore,edgeCost);
            auto handle = openSet.push(
